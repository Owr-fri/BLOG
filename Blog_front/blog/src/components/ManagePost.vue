<template>
  <div class="epost-content">
    ...
    <div>
      ...
      >
        ...
        <el-table-column fixed="right" label="操作" width="120">
          <template slot-scope="scope">
            <el-button @click="view(scope.row)" type="text" size="mini"
              >查看</el-button
            >
    ...
    >
      ...
      <div class="desc">
        <div class="author">作者：{{ this.currentPost.author }}</div>
        <div class="time">发布时间：{{ this.currentPost.publishTime }}</div>
        <div class="category">分类：{{ this.currentPost.categoryName }}</div>
        <div class="label">
          <span> 标签： </span>
          <span
            v-for="(item, index) in this.currentPost.labelName"
            :key="index"
          >
            {{ item }}
          </span>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="danger" @click="editPost = false" plain size="small"
          >取 消</el-button
        >
        <el-button
          type="primary"
...
export default {
  ...
  methods: {
            ...
            axios
              ...
              .then((res) => {
                this.blobToBase64(res).then((result) => {
                  let _file = new File([result], imgName, {
                    type: "image/jpeg",
                  });
    ...
    ImgUpload(pos, $file) {
      ...
      axios
        .post(this.$API.API_UPLOAD_IMG, formdata, {
